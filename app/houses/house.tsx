...
export default function House({ data }: Pops) {
  ...
  const isFiltered =
    maxPrice !== 1000 || bedrooms !== undefined || bathrooms !== undefined;
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-[1fr_26rem] container mx-auto max-w-[1200px] gap-4 px-4 py-8">
        ...
        {/* Filter sidebar */}
        <div className="bg-white h-fit sticky top-5  rounded-2xl shadow p-4 flex flex-col gap-4">
          ...
          {isFiltered && (
            <button
              onClick={() => {
                toast.success("remove the filter !");
